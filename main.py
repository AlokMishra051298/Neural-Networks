...
class Network(object):
  ...
  def feedforward(self,a):
    ...
    for w,b in zip(self.weights, self.biases):
      a=sigmoid(np.dot(w,a)+b)#here we find out the dot product of vectors "w","a" and then add the value to "b"
    return a 
  ...
  def update_mini_batch(self, mini_batch, eta):
    ...
    nabla_b=[np.zeros(b.shape) for b in self.biases]#np.zeroes(shape,order,type) returns ndarray of zeros having given shape, order and datatype.
   ...
   so, ∂z_l/∂w_l = a_(l-1)'''
    ...
    for l in range(2,num_layers):
      ...
      '''here, delta is ∂C_x/∂a_(l-1)=(∂z_l/∂w_l)*(∂C_x/∂a_l)*(∂a_l / ∂z_l)
                                     = w_l * delta
                               '''
      delta=np.dot(self.weight[-l+1].transpose,delta)*sp  #confusion
